...
function playerMapIcon(avatarSrc) {
  return L.icon({
    iconUrl: avatarSrc,
    iconSize: [34, 34],
    iconAnchor: [17, 30],
...
function updateMapPosition(lat, lng, accuracy, avatar) {
  ...
  if (!_selfDot) {
    _selfDot = L.marker(ll, {
      icon: playerMapIcon(avatarSrc),
    })
      .addTo(_map)
      .bindPopup("現在地");
    _selfAvatarSrc = avatarSrc;
  } else {
    _selfDot.setLatLng(ll);
    if (_selfAvatarSrc !== avatarSrc) {
      _selfDot.setIcon(playerMapIcon(avatarSrc));
      _selfAvatarSrc = avatarSrc;
    }
  }
